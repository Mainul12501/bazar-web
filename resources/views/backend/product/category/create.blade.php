                ...
                <div class="card-body">
                    <form action="{{ isset($category) ? route('categories.update', $category->id) : route('categories.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" {{ isset($isShown) ? 'readonly' : '' }} class="form-control" value="{{ isset($category) ? $category->name : '' }}" />
                        </div>
                        <div class="mt-2">
                            <label for="">Description</label>
                            <textarea name="description" {{ isset($isShown) ? 'disabled' : '' }} class="form-control" id="elm1" cols="30" rows="2">{!! isset($category) ? $category->description : '' !!}</textarea>
                        </div>
                        <div class="row mt-2">
                            <div class="col-md-6">
                        ...
                        <div class="mt-2">
                            ...
                            <div>
                                <div class="material-switch">
                                    <input id="someSwitchOptionInfo" name="status" {{ isset($isShown) ? 'disabled' : '' }}  class="form-check-input success check-outline outline-success" type="checkbox" {{ isset($category) && $category->status == 0 ? '' : 'checked' }} />
